...
PLATFORMS = {
    "Google":  "",                    # 一般 Google 搜尋
    "FB":      "site:facebook.com",   # FB 公開頁面
    "IG":      "site:instagram.com",  # IG 公開貼文
    "Threads": "site:threads.net",    # Threads 貼文
    "PTT":     "site:ptt.cc",         # PTT 討論板
}

# ─── 共用 Google Custom Search 函式 ──────────────────────

def search_google_custom(keyword: str, site_prefix: str = "", platform: str = "Google", num: int = 10) -> list[dict]:
    """用 Google Custom Search API 搜尋指定平台"""
    ...
    try:
        ...
        print(f"[{platform} 搜尋錯誤] {keyword}: {e}")
        return []


# ─── Google News RSS（免費備援，無額度限制）───────────────

def search_google_news_rss(keyword: str, num: int = 10) -> list[dict]:
    """爬取 Google News RSS，完全免費不耗 API 額度"""
    results = []
    ...
    try:
        ...
        print(f"[Google News RSS 錯誤] {keyword}: {e}")
    return results
...
def build_email_html(results: list[dict], run_time: str) -> str:
    ...
    platform_summary = " ｜ ".join(f"{k}: {v}筆" for k, v in platform_counts.items())
...
<div style="max-width:800px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1)">
  ...
  <div style="padding:16px 20px;background:#f8fafc;font-size:12px;color:#94a3b8;text-align:center">
    由選情雷達自動產生 · 資料來源：Google、FB、IG、Threads、PTT、Google News
  </div>
</div>
</body></html>"""
...
def main():
    ...
    print(f"監控平台：Google / FB / IG / Threads / PTT / Google News")
    print(f"{'='*50}")
    ...
    for candidate in CANDIDATES:
        ...
        for dimension in DIMENSIONS:
            ...
            # 1. 各平台用 Google Custom Search API 搜尋
            for platform, site_prefix in PLATFORMS.items():
                items = search_google_custom(keyword, site_prefix, platform, num=5)
                print(f"     {platform}: {len(items)} 筆")
                for item in items:
            ...
            # 2. Google News RSS（免費補充，不耗 API 額度）
            news_items = search_google_news_rss(keyword, num=5)
            print(f"     Google News RSS: {len(news_items)} 筆")
    ...
    print(f"\n共蒐集 {len(all_results)} 筆資料，開始情緒分析...")

    # 3. 批次情緒分析（依候選人分組）
    for candidate in CANDIDATES:
        group = [r for r in all_results if r.get("candidate") == candidate and "sentiment" not in r]
        if group:
            analyzed = analyze_sentiment(group, candidate)
            for i, item in enumerate(all_results):
                if item.get("candidate") == candidate and "sentiment" not in item:
                    all_results[i] = group.pop(0)
